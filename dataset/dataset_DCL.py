...
class dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.train:
            ...
            label = self.labels[item]
            return img_unswap, img_swap, label, swap_law1, swap_law2, self.paths[item]
        else:
            label = self.labels[item]
            swap_law2 = [(i-(swap_range//2))/swap_range for i in range(swap_range)]
            return img_unswap, label, swap_law1, swap_law2, self.paths[item]
            ...
            with Image.open(f) as img:
                if self.dataset == 'herb':
                    ...
                    if img_size[0] < img_size[1]:
                        img = img.crop((200, 450, img_size[0]-200, img_size[1]-250))
                    else:
                        img = img.crop((450, 200, img_size[0]-250, img_size[1]-200))
