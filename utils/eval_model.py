...
def eval_turn(model, data_loader, val_version, epoch_num, log_file):
    ...
    with torch.no_grad():
        for batch_cnt_val, data_val in enumerate(data_loader):
            ...
            #law_swap = Variable(torch.from_numpy(np.array(law_swap)).float().cuda())
            # forward
            ...
            loss += ce_loss
            #swap_loss = get_ce_loss(outputs[1], labels_swap).item()
            #loss += 0.2*swap_loss
            #law_loss = get_l1_loss(outputs[2], law_swap).item()
            #loss += 0.2*law_loss
