...
def train(Config,
    ...
    date_suffix = dt()
    log_file = open(os.path.join(Config.log_folder, 'formal_log_r50_dcl_%s_%s_%s.log'%(data_ver, str(data_size), date_suffix)), 'a')
    ...
    for epoch in range(start_epoch,epoch_num-1):
        ...
        for batch_cnt, data in enumerate(data_loader['train']):
            ...
            if Config.use_dcl:
                swap_loss = get_ce_loss(outputs[1], labels_swap)
                loss += swap_loss
            ...
            if Config.use_dcl:
                print('step: {:-8d} / {:d} loss=focal_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} + {:6.4f} + {:6.4f} '.format(step, train_epoch_step, loss.detach().item(), ce_loss.detach().item(), swap_loss.detach().item(), law_loss.detach().item()), flush=True)
            if Config.use_backbone:
                print('step: {:-8d} / {:d} loss=focal_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} '.format(step, train_epoch_step, loss.detach().item(), ce_loss.detach().item()), flush=True)
